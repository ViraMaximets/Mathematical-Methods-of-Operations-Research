...
def find_biggstM(summ):  # шукаємо найбільше по М
    ...
    for i in range(len(summ)):
        ...
        if summ[i] == biggest:
            index.append(i)
    if biggest <= 0:
        print("\nУсі delta задовольняють умову оптимальності!")
        return -1
    return index


def find_biggstC(index):  # якщо є 2 одн М, то шукаємо найбільше по числу
    biggestС = -9999999
    ind_col = -1
    for i in index:
        if sumc[i] > biggestС:
...
def symplex_method(D, b, Cc, Cm, Fc, Fm, ind_row, ind_col):
...
F, A, b = read_matrix()
signs = [-1, 1, 1]  # -1 is <=       0 is =      1 is >=
print("signs: ", signs)

Fc = copy.deepcopy(F)  # stala
Fm = []  # stala
...
D, b, Cc, Cm, basis = symplex_method(D, b, Cc, Cm, Fc, Fm, ind_row, ind_col)
showRes(Fc, basis, b)
